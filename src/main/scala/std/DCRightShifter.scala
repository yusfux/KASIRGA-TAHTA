...
/**
  * DCRightShifter is a shifter from a single numPorts number of ports input interface to a single numPorts number of ports output interface.
  *
  ...
  *  new DCRightShifter(UInt(8.W))(2)
  * }}}
  */
class DCRightShifter[T <: Data](gen: T)(numPorts: Int) extends Module {
  ...
  shifted_in_valids  := BarrelShifter.leftRotate(in_valids, io.shamt)
  shifted_out_readys := BarrelShifter.rightRotate(out_readys, io.shamt)
  shifted_in_bits    := BarrelShifter.leftRotate(in_bits, io.shamt)
