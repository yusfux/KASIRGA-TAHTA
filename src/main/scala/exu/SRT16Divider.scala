...
// https://github.com/OpenXiangShan/XiangShan/tree/master/src/main/scala/xiangshan/backend/fu/SRT16Divider.scala
...
//  val lzcDiff = Mux(state(s_pre_0), lzcWireDiff, lzcRegDiff)
  ...
  mNeg := VecInit(
    Cat(SignExt(MuxLookup(dNormReg(len - 2, len - 4), "b00000000".U(7.W))(mLookUpTable2.minus_m(0)), 11), 0.U(1.W)), // (2, 5) -> (6, 6)
...
//  val rudNegReg = RegEnable(rudNeg, state(s_pre_1))
  ...
  quotIter   := Mux(state(s_iter), quotIterNext, Mux(state(s_pre_1), 0.U(len.W), Mux(quotSignReg, aInverter, quotIterReg)))
  quotM1Iter := Mux(state(s_iter), quotM1IterNext, Mux(state(s_pre_1), 0.U(len.W), Mux(quotSignReg, dInverter, quotM1IterReg)))
