...
// program comitee only wants exception code 0, 1, 2, 3, 4, 5, 6, and 11
// but below is the full list of exceptions that is supported by the RISC-V ISA
object MCAUSE_CODE {
...
class CSR(config: WoodConfig) extends Module {
  ...
  /*TODO: FS should be changed by hardware when FRF is changed
    ...
    CSRList.foreach {
      case (csr, reg, mask) =>
        when(io.csr.addr === csr.U) {
          reg := writeCSR(reg.asUInt, io.csr.wdata, mask, CSROp(io.csr.op)).asTypeOf(reg)
        }
    }
  }
