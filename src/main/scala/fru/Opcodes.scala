...
object Instructions {
  ...
  val ORC_B            = BitPat("b001010000111?????101?????0010011")
  val ORN              = BitPat("b0100000??????????110?????0110011")
  val ROL              = BitPat("b0110000??????????001?????0110011")
  val ROR              = BitPat("b0110000??????????101?????0110011")
  val SEXT_B           = BitPat("b011000000100?????001?????0010011")
  val SEXT_H           = BitPat("b011000000101?????001?????0010011")
  ...
  // zbs32
  val BCLRI_RV32       = BitPat("b0100100??????????001?????0010011")
  val BEXTI_RV32       = BitPat("b0100100??????????101?????0010011")
  val BINVI_RV32       = BitPat("b0110100??????????001?????0010011")
  val BSETI_RV32       = BitPat("b0010100??????????001?????0010011")

  // zbc
  val CLMUL            = BitPat("b0000101??????????001?????0110011")
  val CLMULH           = BitPat("b0000101??????????011?????0110011")
  val CLMULR           = BitPat("b0000101??????????010?????0110011")

  // zbb32
  val REV8_RV32        = BitPat("b011010011000?????101?????0010011")
  val RORI_RV32        = BitPat("b0110000??????????101?????0010011")
  val ZEXT_H_RV32      = BitPat("b000010000000?????100?????0110011")
  ...
  val FSW              = BitPat("b?????????????????010?????0100111")
   ...
   // zba
  val SH1ADD           = BitPat("b0010000??????????010?????0110011")
  val SH2ADD           = BitPat("b0010000??????????100?????0110011")
  val SH3ADD           = BitPat("b0010000??????????110?????0110011")
}
object Causes {
