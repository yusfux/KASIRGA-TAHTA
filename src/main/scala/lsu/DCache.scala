...
class DCache(config: WoodConfig) extends Module {
  ...
  //TODO: idk why but when i try to update with victimWay as index to update the plru tree, it doesn't work
  ...
  val cacheData        = Mux(io.mem.resp.fire && ~isWrite, io.mem.resp.bits.data, wdataReg)
  ...
  switch(state) {
    ...
    is(access) {
      when(isHit) {
        ...
        when(isWrite) {
          state := Mux(isDirty, memWrite, idle)
        }.otherwise {
    ...
    is(memWrite) {
      //TODO: no need to wait the mem response signal for write operation,
      when(io.mem.req.fire) {
        state := Mux(isWrite, idle, memRead)
      }
    }
    is(memRead) {
