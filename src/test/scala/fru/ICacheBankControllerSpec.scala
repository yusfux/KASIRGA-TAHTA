...
class ICacheBankControllerDut(config: WoodConfig) extends Module {
  val io = IO(new Bundle() {
    val core = Vec(
      config.nWide,
      new Bundle() {
        val req  = Flipped(DecoupledIO(new ReadPortI(UInt(config.xlen.W))(config.xlen)))
        val resp = DecoupledIO(new ReadPortO(UInt(config.xlen.W))(config.xlen))
      }
    )

    val data  = Input(UInt(config.mmInterfaceWidth.W))
  ...
  val mem                  = Module(new DecoupledSyncReadBlockRAM(UInt(config.mmInterfaceWidth.W))(new BlockRAMParams(config.mmDepth, 1, 1)))

  mem.io.rip(0).valid                   := icachebankcontroller.io.mem.req.valid
  mem.io.rip(0).bits.addr               := icachebankcontroller.io.mem.req.bits.addr >> (2 + log2Ceil(config.mmInterfaceWidth / 32))
...
class ICacheBankControllerSpec extends AnyFlatSpec with ChiselScalatestTester {
  ...
  val TEST_SIZE = 1024
  val hexArray: Array[String] = (0 until config.mmDepth * 4).map { i =>
    f"${i}%08x"
  }.toArray
